...
<header class='flex shadow-lg py-4 px-4 sm:px-10 bg-white font-[sans-serif] min-h-[70px] tracking-wide relative z-50'>
  <div class='flex flex-wrap items-center justify-between gap-4 w-full'>
    ...
    <div id="collapseMenu"
      ...
      class:d_block={open}>
      <button id="toggleClose" class='lg:hidden fixed top-2 right-4 z-[100] rounded-full bg-white w-9 h-9 flex items-center justify-center border' aria-label="menu" onclick={toggleMenu}>
    ...
    <div class='flex items-center ml-auto space-x-6'>
      <button class='font-semibold text-[15px] border-none outline-none'><a href='/'
      ...
      <button id="toggleOpen" class='lg:hidden' aria-label="menu" onclick={toggleMenu}>
